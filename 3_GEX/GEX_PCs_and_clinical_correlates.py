...
binary_list = ['Recruit_centre','Lupus_status', 'date_processed', 'IFN_status','LN',
                       'Thyroid_Disease_Hashimotos','Graves_Disease','Type_I_Diabetes',
                       'Rheumatoid_Arthritis','MMF','Pred','HCQ','AZA','MTX','TAC','ASP','CYC_ever',
                       'BEL_ever','RTX_ever','OFA_ever','dsDNA','dsDNA_Ab_ever','ANA_ever','C3_status',
                       'C4_status', "Lymphopenia_ever", "Lymphopenia_now"]
...
# %%
continuous_list_small = ["Age_at_sampling", "Years_from_diagnosis","SLEDAI_2K",'SLEDAI_2KG','cSLEDAI_2K',]

binary_list_small = ['Recruit_centre', 'Lupus_status', 'date_processed', 'IFN_status','LN',
                       'Thyroid_Disease_Hashimotos','Graves_Disease','Type_I_Diabetes',
                       'Rheumatoid_Arthritis','MMF','Pred','HCQ','AZA','MTX','TAC','ASP','CYC_ever',
                       'BEL_ever','RTX_ever','OFA_ever','dsDNA','dsDNA_Ab_ever','ANA_ever']
...
# %%
clinical_all["SLEDAI_activity"] = np.where(clinical_all["SLEDAI_2K"] > 4, "ACTIVE", "INACTIVE")

# %%
clinical_all["cSLEDAI_activity"] = np.where(clinical_all["cSLEDAI_2K"] > 4, "ACTIVE", "INACTIVE")

# %%
clinical_all["cSLEDAI_activity"].value_counts()
...
for norm,file in zip(norm_list, file_list):
    ...
    binary_variables = binary_variables[binary_list_small]
    ...
    temp_binary_list_small = binary_variables.columns
    
    # Concatenate continuous variables with the cleaned binary variables

    clinvar_data = pd.concat([all_pcs_all[continuous_list_small], binary_variables], axis = 1)

    # Dictionary of continuous and categorical variables

    clinvars = {"continuous":continuous_list_small, "categorical":temp_binary_list_small }
    ...
    for hue_variable in ["Recruit_centre", "Site", "Ethnicity_cerner", "SLEDAI_2K", 
                         ...
                         "poolID", "Flow_cell", "id_pool_lims", "SLEDAI_activity", "cSLEDAI_activity"]:
        plot_x = "Dim.1"
        plot_y = "Dim.2"
        label_x = f"PC{plot_x.split('.')[1]}"
        label_y = f"PC{plot_y.split('.')[1]}"
    ...
    for hue_variable in ["Age_at_sampling", "Recruit_centre"]:
        plot_x = "Dim.3"
        plot_y = "Dim.4"
        label_x = f"PC{plot_x.split('.')[1]}"
        label_y = f"PC{plot_y.split('.')[1]}"
